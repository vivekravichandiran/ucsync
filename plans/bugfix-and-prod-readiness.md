...
### 19. `uc_sync_state` is unreadable: SUCCESS/FAILURE only + `error_message` overloaded — **FIXED (live-validated 2026-09-14)** (2026-09-13)
1. **Bug:** Found in live testing. Two clarity defects making the state table "a mess": (a) `last_sync_status` had only SUCCESS/FAILURE(/PENDING), so incremental **skips were lumped into SUCCESS** and the roll-up hid what was actually applied; and in the report `_wsmig_status_key` mapped an `UNCHANGED` skip (status "SUCCESS") to **"Created"**. (b) `error_message` was overloaded — on SUCCESS rows it held the applied DDL or a skip reason (e.g. "unchanged since last run"), so an "error message" appeared on non-error rows.
2. **Solution:** (a) fuller `last_sync_status` vocabulary — `SUCCESS` (applied) / `UNCHANGED` (incremental skip) / `ADOPTED` (pre-existing) / `REPORT_ONLY` / `SKIPPED` (filtered) / `MANUAL_ACTION_REQUIRED` / `PENDING` / `FAILURE`, centralised in `sync_state._sync_status_for`; the skip path now emits status `UNCHANGED` (not `SUCCESS`); `_wsmig_status_key` maps `UNCHANGED`→Skipped. (b) split the message channels: a new `detail` column carries informational text (skip reason/DDL) and `error_message` is reserved for real failures (`state_row_from_import`). `detail` added to the state DDL + in-place upgrade columns. (c) **report label consistency (found live):** the per-type sheets used ad-hoc labels (`SUCCESS (UNCHANGED)` vs `ALREADY EXISTS (skipped)`) inconsistent with the Summary, and the wsmig `Updated` status was defined but never emitted. `_render_import_status` now renders the ONE wsmig vocabulary (Created / Created (warning) / **Updated** / Adopted (pre-existing) / Skipped (unchanged) / Deferred / Skipped (no target object) / Manual / FAILED), and a governance/grants change on a pre-existing object reads **Updated** (new `delta.GRANTS_UPDATED` action for grants-only changes; `_wsmig_status_key` maps the change actions to `updated`). Tag ops render APPLIED/FAILED like grants (not the object vocab).
3. **Test approach:** `test_state_status_vocabulary_and_message_split`, `test_unchanged_action_reads_as_skipped_not_created`, updated `test_full_run_seeds_then_unchanged_rerun_is_zero_writes` + report vocab tests (295 green). Live re-validation of the state table + report labels.

### 18. Incremental masks prior failures — an object that FAILED is skipped as UNCHANGED→SUCCESS — **FIXED (live-validated 2026-09-14)** (2026-09-13)
1. **Bug:** Found in live testing — the delta baseline (`{full_name: {ddl_hash, governance_hash, grants}}`) did **not** carry the prior `last_sync_status`, so `delta._decide` classified any source-unchanged object as `UNCHANGED` and skipped it **regardless of whether it succeeded last run**. Proof: after an incremental, `invoices_ext`, `accounts_ext`, and the ai_27 fail-closed negatives (`abac_ext`, `employees_secure`, `ext_masked`) all read `SUCCESS`/"unchanged since last run" while being **absent on target (count 0)** — a green run masking missing objects, and flipping correct fail-closed FAILUREs into bogus SUCCESS.
2. **Solution:** `load_baseline` now selects `last_sync_status` into the baseline; `delta._decide` re-attempts (returns `CREATED_NEW`, re-applies governance + grants) any object whose prior status is a definite non-present state (anything outside `{SUCCESS, ADOPTED, UNCHANGED, REPORT_ONLY}`) even when fingerprints match. A blank/unknown prior status (legacy baseline) is treated as present so the first post-upgrade incremental stays idempotent.
3. **Test approach:** `test_prior_failure_is_reattempted_not_skipped` (re-attempt + CREATE runs), unchanged-rerun test still zero-writes (295 green). Live: reset → baseline (negatives FAILURE) → incremental with unchanged source must **re-attempt** the negatives (stay FAILURE, never flip to SUCCESS) and create the external tables.

### 17. FEAT-4 volume-file copy results are absent from the migration report — **FIXED (live-validated 2026-09-14)** (2026-09-13)
1. **Bug:** Found in live testing — the volume-data copy happens and is recorded (import exit payload `vol_copy_status` + the `uc_sync_volume_files` control table), but the copied/skipped/failed files (and bytes, >5 GB skips) **never appear in the XLSX report** the operator actually reads. Two causes: (a) `report.py` has **no** volume-copy section at all (zero references to volume_copy/copy_summary); (b) in `03_Import.py` the report is built (`build_report_from_file`, ~L242) **before** the volume-copy block runs (~L309), so even the aggregate summary isn't available when the report is written. Net: FEAT-4's work is invisible in the report — a reasonable operator expectation that isn't met.
2. **Solution:** (a) added a "Volume Data Copy" sheet to `report.py` via a `volume_copy_results` kwarg (per-file volume / status / source / target / bytes / message + a FAILED-first roll-up with total files & bytes); (b) reordered `03_Import.py` so the report is written **after** the volume-copy block, passing `volume_copy_report_rows` in; the copy stays best-effort so it never blocks the always-written report (bug #4). The kwarg is optional (backward-compatible; sheet omitted when not supplied).
3. **Test approach:** `test_volume_data_copy_sheet_present_only_when_results_given` (295 green); live re-validation that copied/modified files show in the report on the next incremental run.

### 16. FEAT-4 incremental control table didn't persist — **FIXED** (2026-09-13)
1. **Bug:** Found in live Run 3–6 — the volume copy worked (files + content verified on target), but the incremental control table `uc_sync_volume_files` never persisted, so a re-run re-copied every file. **True root cause (revealed by instrumenting the exit payload):** the notebook built the table name from `cfg.ops_catalog`/`cfg.ops_schema`, which are `from_sources` parameters, **not** `SyncConfig` attributes → `AttributeError` before any SQL ran → the try/except silently fell back to in-memory. My interim "job-cluster Spark vs warehouse" theory was a red herring caused by that swallowed error.
2. **Solution:** (a) still persist via the import **warehouse** executor (`WarehouseVolumeCopyControl`, `RestSqlExecutor` + batched `MERGE USING (VALUES …)`) for FEAT-1 consistency — proven to work on the warehouse; (b) derive the table name from `cfg.state_table` (`{ops}.uc_sync_state` → `{ops}.uc_sync_volume_files`); (c) surface the control kind + copy summary + flush status in the import exit payload so a fallback is never silent again.
3. **Test approach:** `test_warehouse_control_persists_and_skips_across_instances` (291 green) + live confirmation that the control table now populates.
