...
| `connectivity_mode` | `direct` (default) \| `airgap` | 01, 02 | `direct` = job runs on the target and reads/captures the source over a SQL warehouse; `airgap` = run 01+02 on the source (capture over a source SQL warehouse), move the bundle, run 03 on the target. A SQL warehouse is required in **both** modes for export. |
...
| `mapping_file_path` | `/Volumes/…csv` | 02 | Legacy storage-credential + location mapping (back-compat; superseded by `external_locations_path`). |
| `external_locations_path` | `/Volumes/…csv` | 01, 02, 03 | **The single external-storage mapping file.** Column shape auto-selects: **2 cols** (`source_base_path,target_base_path`) → BYO (SC/EL are prerequisites; only prefix-swap external `LOCATION`s, and the SC/EL create toggles are forced off); **3 cols** (`+access_connector_id`) → the utility creates the storage credential + external location. Drives the export path-rewrite AND the import-time base-path swap. |
...
| `source_warehouse_id` | warehouse id | 01, 02 | **SQL warehouse — REQUIRED for export in every mode.** All full-fidelity DDL capture runs on it: `SHOW CREATE` for the table/view family (a classic Spark cluster is flaky on masked/row-filtered tables; a failed capture is a **hard failure** — no synthesized fallback that would drop masks/constraints), and functions from `information_schema`. Also used for governance reads (tags + ABAC) at stage 01 — classic job-cluster Spark cannot read `information_schema.abac_policy_definitions`. Point it at any warehouse on the workspace that owns the objects. |
| `import_warehouse_id` | warehouse id | 03 | **SQL warehouse (target workspace) for the ABAC phase AND the view-creation phase.** `CREATE POLICY` is rejected at parse on a classic Spark cluster; likewise `CREATE VIEW` over a masked/row-filtered base table errors on classic Spark but succeeds on a warehouse. **Required whenever the bundle contains ABAC policies** — otherwise those fail closed (`ABAC_WAREHOUSE_REQUIRED`) and the tables they would have protected are **dropped** — and strongly recommended whenever the bundle has views over masked tables. When unset, views fall back to the Spark executor. |
...
| `object_locations_path` | `/Volumes/…csv` or blank | 03 | Optional **exact per-object** override that BEATS the `external_locations` base-path swap (precedence: exact override → base-path swap → skip). A schema row sets its `MANAGED LOCATION`; an external volume/table row sets that object's `LOCATION`. Blank = rely on `external_locations` / the catalog root. |
| `run_as_spn` | SP application id or blank | 03 | Run the import as a target **service principal** (so migrated securables are owned by it). Blank = run as the installing user. Applies to the import/e2e jobs. |
| `source_run_as_spn` | SP application id or blank | 01+02 | Run the **source** Inventory+Export job as a source-side (read-only) service principal. Blank = run as the installing user. Applies only to the airgap source job. |
| `force_full` | bool (default false) | 03 | Incremental sync is **auto-detected** from `uc_sync_state` (baseline present → incremental, only deltas applied; none → full + seed). Set true to force a full re-seed / reconcile. |
| `migrate_materialized_views` | bool (default false) | 03 | Streaming tables & materialized views are DLT/SDP-pipeline-managed and **report-only** by default (re-issuing their DDL would spin a new pipeline). Set true to migrate materialized views; streaming tables are always report-only. |
...
| `allow_missing_report` | bool (default false) | 03 | When false, report generation is **non-best-effort** — a failure to write the report fails the run (no green run without its report). |
| `create_catalogs`, `create_schemas`, `create_storage_credentials`, `create_external_locations` | bool (**default false** — BYO) | 03 | **BYO-by-default:** the catalog, schemas, storage credential, and external location are customer prerequisites, so their creation is **off by default** — the utility starts *inside the schema*. Turn on for a from-scratch (Mode A) run. A **3-column** `external_locations.csv` turns SC/EL creation back on automatically. When SC/EL creation is off, their grants + ownership are **not** replayed (metastore-scoped, out of the catalog-scoped migration). |
| `create_volumes`, `create_functions`, `create_tables`, `create_views`, `create_abac_policies` | bool (default true) | 03 | Gate **creation** of the schema contents + ABAC. Off = assume pre-existing, skip create, still govern. |
